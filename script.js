...
let rows_amount = getComputedStyle(PLAY_POOL).getPropertyValue('grid-template-rows').split(' ').length;
let columns_amount = getComputedStyle(PLAY_POOL).getPropertyValue('grid-template-columns').split(' ').length;
...
function scoreCount(fRow, fCol) {
	...
	setTimeout(function () {
		SCORE_EFFECT_span.style.opacity = 0;
	}, 500);
	setTimeout(function () {
		SCORE_EFFECT_span.remove();
	}, 1000);
}
function offClick(e) {
...
function tailCollision() {
	...
	for (let i = 0; i < TAILS.length; i++) {
		if (
			pRow == parseInt(getComputedStyle(TAILS[i]).getPropertyValue('grid-row-start')) &&
...
function tailMove(pRow, pCol) {
	if (TAILS.length > 0) {
		...
		TAILS.forEach((element, index) => {
			if (index != 0) {
				row_POSITION.push(parseInt(getComputedStyle(TAILS[index - 1]).getPropertyValue('grid-row-start')));

				col_POSITION.push(parseInt(getComputedStyle(TAILS[index - 1]).getPropertyValue('grid-column-start')));
			}
		});
		...
		TAILS.forEach((element, index) => {
			if (index == 0) {
				...
				element.style.gridRowStart = parseInt(row_POSITION[index - 1]);
				element.style.gridColumnStart = parseInt(col_POSITION[index - 1]);
			}
		});
	}
}
	...
	if (TAILS.length > 1) {
		TAILS.forEach((element, index) => {
			if (index + 1 == TAILS.length) {
				tail.style.gridRowStart = parseInt(getComputedStyle(TAILS[index - 1]).getPropertyValue('grid-row-start'));

				tail.style.gridColumnStart = parseInt(getComputedStyle(TAILS[index - 1]).getPropertyValue('grid-column-start'));
			}
		});
	} else {
...
function foodCollision(pRow, pCol) {
	let fCol = parseInt(getComputedStyle(food).getPropertyValue('grid-column-start'));
	let fRow = parseInt(getComputedStyle(food).getPropertyValue('grid-row-start'));
...
function foodCreate(player_position, pRow, pCol) {
	...
	if (food_position == player_position || (pRow == fRow && pCol == fCol)) {
		...
		for (let i = 0; i < TAILS.length; i++) {
			if (
				fRow == parseInt(getComputedStyle(TAILS[i]).getPropertyValue('grid-row-start')) &&
...
function gameWin(message, time, trigger) {
	...
	let SCORE_sort = SCORES_TABLE.sort((a, b) => (a.score < b.score ? 1 : -1))
		.map(
			(person, index) => `
	...
	high_scores.players = SCORES_TABLE.filter((person, index) => index < 10);
...
RESET_SETT_input.addEventListener(
	'click',
	function () {
		setTimeout(resetSettings);
	},
	15
);
